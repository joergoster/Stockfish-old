...
Value Entry::evaluate_shelter(const Position& pos, Square ksq) {
  ...
  Bitboard b = ~forward_ranks_bb(Them, ksq);
  Bitboard ourPawns   = b & pos.pieces(Us  , PAWN);
  Bitboard theirPawns = b & pos.pieces(Them, PAWN);

  Value safety = shift<Down>(theirPawns) & RookFiles & BlockRanks & ksq ? Value(374) : Value(5);
  ...
  for (File f = File(center - 1); f <= File(center + 1); ++f)
  {
      ...
      safety -= (ourRank && (ourRank == theirRank - 1)) ? theirRank == RANK_3 ? 66 : 0
